...
- Autenticação própria por CNPJ/e-mail e senha, sessões seguras em cookie `httpOnly`, perfis `ADMIN` e `COURIER`.
...
- O vínculo de CNPJ usa comparação conservadora de nomes entre as duas guias; casos incertos entram em uma fila de conciliação manual.
- Fotos de produtos aceitarão upload e URL, com limites e validação.

## Módulos

1. **Autenticação**: cadastro vinculado ao CNPJ conciliado, login, logout, recuperação e redefinição de senha.
...
- CNPJ é armazenado normalizado e único no cadastro de entregadores.
- Um cadastro só é permitido se o CNPJ estiver conciliado, ainda não possuir conta e o código de ativação for válido.
...
- Cadastrar e autenticar um CNPJ conciliado.
