    ...
    <article className="rounded-[20px] border border-slate-200 bg-white p-5 shadow-sm ring-1 ring-slate-950/[0.02] hover:-translate-y-0.5 hover:shadow-md transition-all duration-200">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 truncate text-2xl font-extrabold tracking-tight tabular-nums text-[var(--brand-navy)] sm:text-3xl">{value}</p>
          {helper ? <p className="mt-1 text-pretty text-xs leading-5 text-slate-600 font-medium">{helper}</p> : null}
        </div>
