...
export type ReconciliationCourier = { id: string; name: string; cnpj: string | null; plaza: string | null };
        ...
        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-[20px] bg-white p-6 shadow-xl focus:outline-none">
          ...
          <form onSubmit={reconcile} className="mt-6 space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <section className={selected ? "rounded-2xl border border-blue-200 bg-blue-50 p-4" : "rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4"} aria-labelledby={`target-${entry.id}`}><p className="text-xs font-bold text-[var(--brand-blue-dark)]">Destino: entregador</p>{selected ? <><h3 id={`target-${entry.id}`} className="mt-2 text-balance font-extrabold text-[var(--brand-navy)]">{selected.name}</h3><p className="mt-2 text-sm font-bold tabular-nums text-slate-700">{formatCnpj(selected.cnpj)}</p><p className="mt-1 text-xs text-slate-500">{selected.plaza || "Praça não informada"}</p></> : <p id={`target-${entry.id}`} className="mt-4 text-pretty text-sm leading-6 text-slate-500">Selecione um entregador para comparar os dados.</p>}</section>
            </div>
