        ...
        <AlertDialog.Content className="fixed left-1/2 top-1/2 z-50 max-h-[calc(100dvh-2rem)] w-[calc(100%-2rem)] max-w-2xl -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-[20px] bg-white p-6 shadow-xl focus:outline-none">
          ...
          <form onSubmit={reconcile} className="mt-6 space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <section className="rounded-2xl border border-amber-200 bg-amber-50 p-4" aria-labelledby={`source-${entry.id}`}><p className="text-xs font-bold text-amber-900">Origem: DADOS CNPJ</p><h3 id={`source-${entry.id}`} className="mt-2 text-balance font-extrabold text-[var(--brand-navy)]">{entry.sourceName}</h3><p className="mt-2 text-sm font-bold tabular-nums text-amber-950">{formatCnpj(entry.cnpj)}</p><p className="mt-1 text-xs tabular-nums text-amber-800/80">Linha {entry.sourceRow}</p></section>
              <section
                className={
                  selected
                ...
                aria-labelledby={`target-${entry.id}`}
              >
                <p className="text-xs font-bold text-[var(--brand-blue-dark)]">Destino: entregador</p>
                {selected ? (
                  <>
                    <h3 id={`target-${entry.id}`} className="mt-2 text-balance font-extrabold text-[var(--brand-navy)]">
                      {selected.name}
                    </h3>
                    <p className="mt-2 text-sm font-bold tabular-nums text-blue-900">
                      {formatCnpj(selected.cnpj)}
                    </p>
                  </>
                ) : (
                  <p id={`target-${entry.id}`} className="mt-4 text-pretty text-sm leading-6 text-slate-600">
                    Selecione um entregador para comparar os dados.
                  </p>
                )}
              </section>
            </div>
